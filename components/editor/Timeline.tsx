  ...
  return (
    <div
      ...
      className={`
        ...
        ${isSelected ? 'border-[var(--funeral-primary)]' : 'border-[var(--neutral-deep-highlight)] hover:border-[var(--funeral-primary)]/40'}
      `}
      onClick={onSelect}
    >
      {/* Drag handle */}
      ...
      {/* Thumbnail */}
      <div className="aspect-video bg-[var(--neutral-deep-highlight)] flex items-center justify-center">
        {slide.mediaType === 'image' ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={slide.mediaUrl} alt={`Slide ${index + 1}`} className="w-full h-full object-cover" />
        ) : (
          <div className="flex flex-col items-center gap-1">
            <svg className="w-6 h-6 text-[var(--neutral-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 10l4.553-2.069A1 1 0 0121 8.87v6.26a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
            </svg>
            <span className="text-xs text-[var(--neutral-muted)]">Video</span>
          </div>
        )}
      </div>

      {/* Info bar */}
      <div className="bg-[var(--neutral-highlight)] px-2 py-1 flex justify-between items-center">
        <span className="text-xs text-[var(--neutral-muted)]">#{index + 1}</span>
        <span className="text-xs text-[var(--neutral-muted)]">{slide.duration}s</span>
      </div>
...
export default function Timeline({ selectedId, onSelect }: TimelineProps) {
  ...
  if (!slides.length) {
    return (
      <div className="h-28 flex items-center justify-center border border-dashed border-[var(--neutral-deep-highlight)] rounded-xl">
        <p className="text-[var(--neutral-muted)] text-sm">Upload photos or videos to get started</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      {previewStatus === 'stale' && (
        <div className="bg-[var(--funeral-primary)]/10 border border-[var(--funeral-primary)]/30 rounded-lg px-3 py-2 text-sm text-[var(--funeral-primary)] flex items-center gap-2">
